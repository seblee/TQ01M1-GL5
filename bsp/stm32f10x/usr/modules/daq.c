...
uint16_t get_current_hum()
{
    ...
    {
        if (((g_sys.config.dev_mask.mb_comp) & (0X01 << i)) && (sys_get_mbm_online(i) == 1))  //配置位 online
        {
...
static int16_t Calc_UV_ai(uint16_t adc_value, uint16_t k_factor, int16_t cali)
{
    ...
    ret_val = ((3.3 * adc_value * 100 * 4) / (4096 * 5) - 2) * 1000 * 10 / k_factor +
              (int16_t)(cali);  // unit is BAR(aka. 0.1MPaG)
    if (ret_val <= 0)
    {
        ret_val = ABNORMAL_VALUE;
    }
...
void ai_sts_update(sys_reg_st *gds_sys_ptr)
{
    ...
    {
        gds_sys_ptr->status.ComSta.u16Ain[i] =
            (((ain_mask_bitmap >> i) & 0x0001) != 0)
                ? calc_ntc(u16ADCRemapValue[i], gds_sys_ptr->config.general.ntc_cali[i - AI_NTC1])
                : 0;
    }

    if ((ain_mask_bitmap & (0x0001 << AI_SENSOR1)) != 0)
    {
        gds_sys_ptr->status.ComSta.u16Ain[AI_SENSOR1] =
            Calc_UV_ai(u16ADCRemapValue[AI_SENSOR1], K_FACTOR_UV, gds_sys_ptr->config.general.ai_cali[AI_SENSOR1]);
    }

    // rt_kprintf("Sensor:%d,NTC1:%d,NTC2:%d,NTC3:%d,NTC4:%d,NTC5:%d,NTC6:%d\n", gds_sys_ptr->status.ComSta.u16Ain[0],
    // gds_sys_ptr->status.ComSta.u16Ain[1], gds_sys_ptr->status.ComSta.u16Ain[2], gds_sys_ptr->status.ComSta.u16Ain[3],
    // gds_sys_ptr->status.ComSta.u16Ain[4], gds_sys_ptr->status.ComSta.u16Ain[5],
    // gds_sys_ptr->status.ComSta.u16Ain[6]);

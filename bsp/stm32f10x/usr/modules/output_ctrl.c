...
static void oc_do_update(uint32_t new_bitmap)
{
    ...
    //		xor_bitmap = new_bitmap ^ old_bitmap;
    ////	  rt_kprintf("g_sys.status.dout_bitmap= %x,old_bitmap=
    ///%x,xor_bitmap=%x,new_bitmap=%x\n",g_sys.status.dout_bitmap,old_bitmap,xor_bitmap,new_bitmap);
    //		if(xor_bitmap != 0)																					//if
    // output bitmap changed
    //		{
    //				for(i=0;i<16;i++)
    //				{
...
static uint32_t oc_arbitration(void)
{
    ...
    for (u8i = 0; u8i <= 1; u8i++)
    {
        ...
        // if diagnose enable, output manual, else out put concatenated bitmap
        {
        ...
        final_bitmap = (g_sys.config.ComPara.u16Test_Mode_Type == 0) ? sys_bitmap : l_sys.bitmap[u8i][BITMAP_MANUAL];
        // final bitmap selection, if test mode enable, output
...
static void oc_ao_arbitration(void)
{
    ...
    if ((g_sys.config.ComPara.u16Manual_Test_En == 0) &&
        (g_sys.config.ComPara.u16Test_Mode_Type ==

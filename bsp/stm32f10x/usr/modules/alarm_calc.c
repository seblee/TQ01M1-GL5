...
const uint16_t ACL_CONF[ACL_TOTAL_NUM][ALARM_ACL_MAX] =
    //	id ,en_mask,reless_mask,DEV_type
    {
        ...
        17, ACL_ENMODE_OTHER, ACL_ENMODE_HAND_RESET_ALARM, MIOOR_ALARM_LEVEL, DEV_TYPE_OTHER,    //ACL_E17
...
static uint16_t acl11(alarm_acl_status_st *acl_ptr)
{
    ...
    if (acl_clear(acl_ptr))
    {
		...
		if ((Exhaust_Temp > (g_sys.config.alarm[ACL_E11].alarm_param - 50)))
    {
        data = ALARM_ACL_HOLD;
    }
    else
